...
void MomCorr_e1f::read_mom_par() {
  ...
  for (Int_t s=1; s<=NSect; s++) {
    ...
    for (Int_t k=0; k<2; k++) {
      ...
      sprintf(file, "/u/home/dmriser/mydoc/analysis/root_scripts/analysisClasses/momCorr/momentum2_s%d_c%d.out", s, k);
      FILE *fp = fopen(&file[0], "r");
      if (fp) {
...
void MomCorr_e1f::read_mom_pip_par() {
  ...
  for (Int_t s=1; s<=NSect; s++) {
    ...
    for (Int_t k=0; k<2; k++) {
      ...
      sprintf(file, "/u/home/dmriser/mydoc/analysis/root_scripts/analysisClasses/momCorr/momentum3_s%d_c%d.out", s, k);
      FILE *fp = fopen(&file[0], "r");
      if (fp) {
