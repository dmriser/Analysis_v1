...
void change_type()
{
  ...
  string sType[3] = {"GSIM NO Radiative.","GSIM + Radiative.","Data."};
  ...
  for (int s=0; s<7; s++)
    {
      ...
      histos.h2_xs[s]     = (TH2F*) histos.h2_rec[2][s]->Clone();
      ...
      for (int t=0; t<thetaBins.number(); t++)
	{
      ...
      histos.h1_xs_ratio[1][s] = (TH1F*) histos.h1_xs_raw[s]->Clone();
...
void do_norm_theta()
{
  cout << " Doing normalized theta distributions... ";

  for (int i=0; i<2; i++) 
    for (int k=0; k<7; k++)
      {
	histos.h1_norm_theta[i][0][k] = (TH1F*) histos.h1_gen_theta[i][k]->Clone();
	histos.h1_norm_theta[i][1][k] = (TH1F*) histos.h1_rec_theta[i][k]->Clone();
	histos.h1_norm_theta[i][2][k] = (TH1F*) histos.h1_rec_theta[2][k]->Clone();

	histos.h1_norm_theta[i][0][k]->SetName(Form("h1_norm_theta_gen_%s_%s",T[i].c_str(),S[k].c_str()));
	histos.h1_norm_theta[i][1][k]->SetName(Form("h1_norm_theta_rec_%s_%s",T[i].c_str(),S[k].c_str()));
	histos.h1_norm_theta[i][2][k]->SetName(Form("h1_norm_theta_DATA_%s_%s",T[i].c_str(),S[k].c_str()));

	histos.h1_norm_theta[i][0][k]->Scale(1/histos.h1_norm_theta[i][0][k]->Integral());
	histos.h1_norm_theta[i][1][k]->Scale(1/histos.h1_norm_theta[i][1][k]->Integral());
	histos.h1_norm_theta[i][2][k]->Scale(1/histos.h1_norm_theta[i][2][k]->Integral());
      }
  cout << " Done! " << endl;
